...
while ( my $line = readline $fh_build ) {
    ...
    $build{Term_Size_Win32}    = $1 if $line =~ /^\s*'?Term::Size::Win32'?\s*=>\s*([^\s,]+)/;
    $build{Text_LineFold}      = $1 if $line =~ /^\s*'?Text::LineFold'?\s*=>\s*([^\s,]+)/;
    $build{Unicode_GCString}   = $1 if $line =~ /^\s*'?Unicode::GCString'?\s*=>\s*([^\s,]+)/;
    $build{Win32_Console}      = $1 if $line =~ /^\s*'?Win32::Console'?\s*=>\s*([^\s,]+)/;
    $build{Win32_Console_ANSI} = $1 if $line =~ /^\s*'?Win32::Console::ANSI'?\s*=>\s*([^\s,]+)/;
}
close $fh_build;
...
while ( my $line = readline $fh_make ) {
    ...
    $make{Term_Size_Win32}    = $1 if $line =~ /^\s*'?Term::Size::Win32'?\s*=>\s*([^\s,]+)/;
    $make{Text_LineFold}      = $1 if $line =~ /^\s*'?Text::LineFold'?\s*=>\s*([^\s,]+)/;
    $make{Unicode_GCString}   = $1 if $line =~ /^\s*'?Unicode::GCString'?\s*=>\s*([^\s,]+)/;
    $make{Win32_Console}      = $1 if $line =~ /^\s*'?Win32::Console'?\s*=>\s*([^\s,]+)/;
    $make{Win32_Console_ANSI} = $1 if $line =~ /^\s*'?Win32::Console::ANSI'?\s*=>\s*([^\s,]+)/;
}
close $fh_make;
